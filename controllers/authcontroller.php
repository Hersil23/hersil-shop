...
class AuthController {
    ...
    public function register() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($correo) || !isValidEmail($correo)) $errors[] = "El correo electrónico no es válido";
            if (strlen($contrasena) < 8) $errors[] = "La contraseña debe tener al menos 8 caracteres";
            if ($contrasena !== $confirmar_contrasena) $errors[] = "Las contraseñas no coinciden";

            $this->userModel->correo = $correo;
            if ($this->userModel->emailExists()) $errors[] = "Este correo ya está registrado";
